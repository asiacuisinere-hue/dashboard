    ...
    useEffect(() => {
        const fetchKpis = async () => {
            ...
            try {
                ...
                const formattedRevenueData = (data.revenueData || []).map(item => ({ name: new Date(item.day).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' }), ca: parseFloat(item.revenue) }));
                setRevenueData(formattedRevenueData);
                
                const typeLabels = { 'COMMANDE_MENU': 'Menus', 'COMMANDE_SPECIALE': 'Spéciales', 'RESERVATION_SERVICE': 'Réservations', 'SOUSCRIPTION_ABONNEMENT': 'Abonnements' };
                ...
                const formattedOrderTypeData = (data.orderTypeData || []).map(item => ({ name: typeLabels[item.type] || item.type, value: Number(item.count), color: colorMapping[item.type] || '#6b7280' }));
                setOrderTypeData(formattedOrderTypeData);

                const formattedWeekdayData = (data.weekdayData || []).map(item => ({ day: item.day_name, commandes: item.total_orders, ca: parseFloat(item.total_revenue) }));
                setWeekdayData(formattedWeekdayData);

                const formattedTopProducts = (data.topProductsData || []).map(item => ({ name: item.item_name, orders: item.total_orders, revenue: parseFloat(item.total_revenue), avgRevenue: parseFloat(item.average_revenue) }));
                setTopProducts(formattedTopProducts);

                const formattedExpenseData = (data.expenseDistributionData || []).map(item => ({ name: item.name, value: Number(item.value) }));
                setExpenseDistributionData(formattedExpenseData);

                const formattedMonthlyPerformanceData = (data.monthlyPerformanceData || []).map(item => ({ name: new Date(item.month_start).toLocaleDateString('fr-FR', { year: '2-digit', month: 'short' }), commandes: item.total_orders, ca: parseFloat(item.total_revenue) }));
                ...
                setKpis({ revenue: '0.00', revenueChange: 0, totalExpenses: '0.00', expensesChange: 0, totalGrossMargin: '0.00', grossMarginChange: 0, orders: 0, ordersChange: 0, newClients: 0, clientsChange: 0, avgOrderValue: '0.00' });
                setRevenueData([]); setOrderTypeData([]); setWeekdayData([]); setTopProducts([]); setExpenseDistributionData([]); setMonthlyPerformanceData([]); setEventsData([]);
            } finally {
                setLoading(false);
            }
        };
                    ...
                    <div className="flex flex-wrap gap-2">
                        {['last7days', 'last30days', 'currentMonth', 'currentYear'].map((p) => (
                            <button
                                ...
                                onClick={() => setPeriod(p)}
                                disabled={loading}
                                className={`px-4 py-2 rounded-lg font-medium transition-colors ${ period === p ? 'bg-amber-500 text-white shadow-md' : 'bg-white text-amber-500 border border-amber-500 hover:bg-amber-50' } ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                            >
                                {p === 'last7days' && '7 jours'}
