...
const DashboardLayout = () => {
    ...
    const [draftQuotesCount, setDraftQuotesCount] = useState(0);
    ...
    const fetchCounts = useCallback(async () => {
        ...
        // FIXED: Count only confirmed/paying for "En cours" (exclude draft/awaiting review/sent quotes)
        const { count: inProgressDemandsCount } = await supabase
            .from('demandes')
            .select('*', { count: 'exact', head: true })
            .eq('business_unit', businessUnit)
            .or(`and(type.in.("COMMANDE_MENU","COMMANDE_SPECIALE"),status.in.("confirmed","En attente de paiement","En cours de traitement")),and(type.eq.RESERVATION_SERVICE,status.in.("confirmed","En attente de paiement","En cours de traitement"))`);
        ...
        const { count: draftQuotes } = await supabase.from('quotes').select('*', { count: 'exact', head: true }).eq('status', 'draft').eq('business_unit', businessUnit);
        setDraftQuotesCount(draftQuotes || 0);
